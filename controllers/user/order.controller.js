...
exports.createOrderProcess = async (req, res) => {
    ...
    if (error) {
        req.log.warn(`Validation failed for order creation (user ${userId}): ${error.details[0].message}`);
    ...
    try {
        const result = await db.sequelize.transaction(async (t) => {
            ...
            const orderDetailsData = cart.cartDetails.map(item => ({
                ...
                price_at_order: item.price_at_addition, product_name: item.product.name
            }));
            ...
            const order = await Order.create({
                ...
                shipping_address: value.shipping_address, billing_address: value.billing_address,
                payment_method: value.payment_method, notes: value.notes
            }, { transaction: t });

             const orderDetailsForCreation = orderDetailsData.map(item => ({ ...item, order_id: order.id }));
...
exports.listOrders = async (req, res) => {
    ...
    try {
        ...
        res.render("user/orders/index", { title: "Đơn hàng của bạn", orders: data });
    } catch (err) {
...
exports.orderDetail = async (req, res) => {
    ...
    try {
        ...
        if (data) { res.render("user/orders/detail", { title: `Chi tiết Đơn hàng #${data.id}`, order: data }); }
        else { req.log.warn(`Order not found for user ${userId}: ${orderId}`); res.status(404).render("user/errorPage", { title: "Không tìm thấy", message: `Không tìm thấy Đơn hàng với id=${orderId}.` }); }
    } catch (err) {
