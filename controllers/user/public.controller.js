...
exports.home = async (req, res) => { // Sử dụng async
    req.log.info('Rendering public home page');
    try { // Bắt đầu try block
        const [latestProducts, categories] = await Promise.all([ // Sử dụng await và Promise.all
...
exports.listProducts = async (req, res) => { // Sử dụng async
    ...
    try { // Bắt đầu try block
        const products = await Product.findAll({ // Sử dụng await
            where: condition,
            include: ['category']
            // Thêm order, limit, offset cho phân trang, sắp xếp
        });

        const categories = await Category.findAll(); // Sử dụng await
...
exports.productDetail = async (req, res) => { // Sử dụng async
     const id = req.params.productId;
     req.log.info(`Rendering public product detail for ID: ${id}`);
     try { // Bắt đầu try block
       const data = await Product.findByPk(id, { include: ['category'] }); // Sử dụng await
        if (data) {
